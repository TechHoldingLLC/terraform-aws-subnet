...
resource "aws_network_acl_association" "nacl_association" {
  for_each       = { for subnet in aws_subnet.private_subnet : "${subnet.availability_zone}-${subnet.cidr_block}" => subnet }
...
resource "aws_network_acl_rule" "ingress_nacl_rule" {
  for_each       = { for index, rule in local.nacl_ingress : "${var.name}-${rule.rule_number}-${rule.egress}" => rule }
  network_acl_id = aws_network_acl.nacl.id
...
resource "aws_network_acl_rule" "egress_nacl_rule" {
  for_each       = { for index, rule in local.nacl_egress : "${var.name}-${rule.rule_number}-${rule.egress}" => rule }
  network_acl_id = aws_network_acl.nacl.id
  rule_number    = lookup(each.value, "rule_number")
  egress         = lookup(each.value, "egress")
  protocol       = lookup(each.value, "protocol")
